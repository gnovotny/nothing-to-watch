...
#define EDGE_SMOOTHNESS_BASE 0.0075
#define EDGE_SMOOTHNESS_SCALING 0
//#define EDGE_SMOOTHNESS_MIN 0.
//#define EDGE_SMOOTHNESS_MAX 1.
#define EDGE_THICKNESS_BASE 0.135
#define EDGE_THICKNESS_SCALING 0
//#define EDGE_THICKNESS_MIN 0.
//#define EDGE_THICKNESS_MAX 1.
#define EDGE_ROUNDNESS_BASE 0.055
#define EDGE_ROUNDNESS_SCALING 1
//#define EDGE_ROUNDNESS_MIN 0.
//#define EDGE_ROUNDNESS_MAX 1.
...
vec2 forceCenter;
vec2 forceCenterCoords;
void initForceCenter() {
    forceCenter = rawCoords(fForceCenter);
    forceCenterCoords = aspectCoords(forceCenter);
}
...
void applyMediaBboxFisheye(inout vec2 cellNCoords, inout float reciprocalMediaFisheyeFactor, in vec2 cellCoords, in float fisheyeFactor) {
    if (fFishEyeStrength == 0. || fFishEyeRadius == 0.) return;
...
void processNeighborEdge(in uint neighborIndex, in vec2 cellCoords, in vec2 p, inout vec2 edge, in float weight, in float weightOffset, in float weightOffsetScale, in float edgeRoundness, in float fisheyeFactor) {
...
//        vec2 aspectCorrectedOffset = offset * vec2(1.0 / scaleX, 1.0);
        ...
        edgeRoundness *= len *.5 + .5;
    #endif

    // todo could modify len based on:  * (1./fisheyeFactor)
    edge.x = cSmin(edge.x, len, edgeRoundness);
    edge.y = min(edge.y, len);
}

void calcEdge(in uvec4 indices, in uvec4 indices2, in vec2 cellCoords, in vec2 p, inout vec2 edge, in float weight, in float weightOffset, in float weightOffsetScale, in float edgeRoundness, in float fisheyeFactor) {

    processNeighborEdge(indices.y, cellCoords, p, edge, weight, weightOffset, weightOffsetScale, edgeRoundness, fisheyeFactor);
    processNeighborEdge(indices.z, cellCoords, p, edge, weight, weightOffset, weightOffsetScale, edgeRoundness, fisheyeFactor);
    processNeighborEdge(indices.w, cellCoords, p, edge, weight, weightOffset, weightOffsetScale, edgeRoundness, fisheyeFactor);

    #if DOUBLE_INDEX_POOL == 1 && DOUBLE_INDEX_POOL_EDGES == 1
        processNeighborEdge(indices2.x, cellCoords, p, edge, weight, weightOffset, weightOffsetScale, edgeRoundness, fisheyeFactor);
        processNeighborEdge(indices2.y, cellCoords, p, edge, weight, weightOffset, weightOffsetScale, edgeRoundness, fisheyeFactor);
        processNeighborEdge(indices2.z, cellCoords, p, edge, weight, weightOffset, weightOffsetScale, edgeRoundness, fisheyeFactor);
        processNeighborEdge(indices2.w, cellCoords, p, edge, weight, weightOffset, weightOffsetScale, edgeRoundness, fisheyeFactor);
    #endif

    #if EDGE_SMIN_SCALING == 1 && EDGE_SMIN_SCALING_COMPENSATION == 1
        // Totally empirical compensation for smoothing scaling side-effect.
        edge.x *= .5 + edgeRoundness;
    #endif
...
Plot plot() {
    ...
    float cellScale = 0.1;
    float edgeThicknessScale = 0.1;
    float edgeSmoothnessScale = 0.1;
    float edgeRoundnessScale = 0.1;
    #if CELL_SCALING == 1
        ...
        #endif
        #if EDGE_THICKNESS_SCALING == 1
            edgeThicknessScale = cellScale;
        #endif
        #if EDGE_THICKNESS_SCALING == 1
            edgeSmoothnessScale = cellScale;
        #endif
        #if EDGE_ROUNDNESS_SCALING == 1
            edgeRoundnessScale = cellScale;
        #endif
    #endif

    float edgeThickness = EDGE_THICKNESS_BASE * edgeThicknessScale * resolutionScale * numCellsScale;
    #if defined(EDGE_THICKNESS_MIN) && defined(EDGE_THICKNESS_MAX)
        edgeThickness = clamp(edgeThickness, EDGE_THICKNESS_MIN * resolutionScale * numCellsScale, EDGE_THICKNESS_MAX * resolutionScale * numCellsScale);
    #endif

    float edgeSmoothness = EDGE_SMOOTHNESS_BASE * edgeSmoothnessScale * resolutionScale * numCellsScale;
    #if defined(EDGE_SMOOTHNESS_MIN) && defined(EDGE_SMOOTHNESS_MAX)
        edgeSmoothness = clamp(edgeSmoothness, EDGE_SMOOTHNESS_MIN * resolutionScale * numCellsScale, EDGE_SMOOTHNESS_MAX * resolutionScale * numCellsScale);
    #endif

    float edgeRoundness = EDGE_ROUNDNESS_BASE * 1./inversesqrt(edgeRoundnessScale) * fEdgeRoundnessMod * resolutionScale;
    #if defined(EDGE_ROUNDNESS_MIN) && defined(EDGE_ROUNDNESS_MAX)
        edgeRoundness = clamp(edgeRoundness, EDGE_ROUNDNESS_MIN * fEdgeRoundnessMod * resolutionScale, EDGE_ROUNDNESS_MAX * fEdgeRoundnessMod * resolutionScale);
    #endif
    ...
    vec2 edge = vec2(0.1);
    calcEdge(indices, indices2, cellCoords, p, edge, weight, weightOffset, weightOffsetScale, edgeRoundness, fisheyeFactor);

    float edgeStep = smoothstep(edgeSmoothness, edgeThickness, edge.x);
...
//    float smoothness = EDGE_SMOOTHNESS_BASE * numCellsScale;
//    float thickness = EDGE_THICKNESS_BASE * numCellsScale;
