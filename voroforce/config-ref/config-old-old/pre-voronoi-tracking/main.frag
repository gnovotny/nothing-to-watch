...
//#define GLOBAL_MAX_NEIGHBOR_ITERATIONS 48
#define GLOBAL_MAX_NEIGHBOR_ITERATIONS 56
#define DEBUG_MEDIA_BBOXES 0
//#define Y_FACTOR 4.5
#define Y_FACTOR 1.
#define MEDIA_UV_ROTATE_FACTOR 1
#define ROUNDNESS 0.001
...
//int neighborsTexData(int index) {
//    int textureWidth = textureSize(uCellNeighborsAltTexture, 0).x;
//    return int(texelFetch(uCellNeighborsAltTexture, ivec2(index % textureWidth, index / textureWidth), 0).r);
//}

//int neighborsAltTexData(int index) {
//    int textureWidth = textureSize(uCellNeighborsAltTexture, 0).x;
//    return int(texelFetch(uCellNeighborsAltTexture, ivec2(index % textureWidth, index / textureWidth), 0).r);
//}
...
vec3 getMediaColor(int cellId, vec4 mediaBbox) {
    ...
    mediaTileUv.y = 1. - mediaTileUv.y;
        ...
        float angle = calculateOrientation(getNormalizedCellCoords(neighborsTexData(nIndexAndLength.x+3)),getNormalizedCellCoords(neighborsTexData(nIndexAndLength.x+4)));
...
//    p.y /= 3.5;
    ...
    if (iFocusedIndex != -1) {
        if (ids.x != iFocusedIndex) {
            ...
            if (distToFocusedCell < 0.25) {
                maxNeighborIterations = min(nIndexAndLength.y, GLOBAL_MAX_NEIGHBOR_ITERATIONS);
//                discard;
            } else if (distToFocusedCell < 0.5) {
                maxNeighborIterations = min(nIndexAndLength.y, 24);
            }
        } else {
            maxNeighborIterations = min(nIndexAndLength.y, GLOBAL_MAX_NEIGHBOR_ITERATIONS);
        }
...
//    nIndexAndLength = ivec2(neighborsTexData(ids.x*2),neighborsTexData(ids.x*2+1));
    nIndexAndLength = ivec2(neighborsAltTexData(ids.x*2),neighborsAltTexData(ids.x*2+1));


    float avgXNeighborLen = (getCellCoords(neighborsAltTexData(nIndexAndLength.x+4)).x - getCellCoords(neighborsAltTexData(nIndexAndLength.x+3)).x) * 0.5;
    float scaleMod = avgXNeighborLen*100.;

//    for (int i = 0; i < min(nIndexAndLength.y, texWeight > 0.05 ? iFocusedIndex == ids.x ? 56 : 56 : 8); i++) {
    for (int i = 0; i < maxNeighborIterations; i++) {
//        int neighborId = neighborsTexData(nIndexAndLength.x+i);
        int neighborId = neighborsAltTexData(nIndexAndLength.x+i);
        vec2 neighborCellCoords = getCellCoords(neighborId);
...
//            minEdgeDist = smin( minEdgeDist, newLen, ROUNDNESS );
            minEdgeDist = smin( minEdgeDist, newLen, scaleMod * ROUNDNESS );
//        }
...
//        mediaBbox.zw = avgCenter + vec2(bbX,bbY)*0.5;
    ...
    return Data(ids, vec2(cellDist, cell2Dist), minEdgeDist, mediaBbox, debugFlag, scaleMod);
}
...
Data initData(vec2 p) {
    ...
    return Data(ivec2(id, id2), vec2(minDist, secondMinDist),  0., vec4(0.), false, 0.);
}
...
//    float mod = 1.;
    ...
    float edge1 = .005 * min(data.scaleMod, 1.);
    float edge2 = .001 * min(data.scaleMod, 1.);

    c = mix(
        c,
        vec3(0.),
        smoothstep(edge1, edge2, data.minEdgeDist)
