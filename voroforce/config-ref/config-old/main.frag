...
layout(location = 0) out vec4 voroBufferColor;
...
//        uint seed = uint(fragCoord.x) + uint(fragCoord.y);
    ...
    for (uint i = 1u; i < 4u; i++) {
        ...
        minEdgeDist = smin2(minEdgeDist, newLen, (newLen*.5 + .5)*fRoundnessMod*ROUNDNESS);
    ...
    return Data(indices, minEdgeDist, mediaBbox, debugFlag, 0.);
}
...
Data initData(vec2 p) {
    ...
    return Data(indices, 0., vec4(0.), false, 0.);
}
...
//    );
    ...
    voroBufferColor = uintBitsToFloat(indices + 1u);
