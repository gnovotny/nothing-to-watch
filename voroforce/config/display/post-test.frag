...
//    ew *= fEdgeStrength;
    ...
    float maxScale = 0.025;
//    scaleMod = min(abs(scaleMod - maxScale)/maxScale, 1.); // Normalize the scale domain to a range of zero to one.
    scaleMod = abs(scaleMod - maxScale)/maxScale; // Normalize the scale domain to a range of zero to one.
